...
def stub_db_functions():
    patch("services.library_service.calculate_late_fee_for_book", return_value={"fee_amount": 10.0, "days_overdue": 5, "status": "Success"}).start()
    patch("services.library_service.get_book_by_id", return_value={"id": 1, "title": "Test Book"}).start()

def stub_zero_fee():
    patch("services.library_service.calculate_late_fee_for_book", return_value={"fee_amount": 0.0, "days_overdue": 0, "status": "Success"}).start()
